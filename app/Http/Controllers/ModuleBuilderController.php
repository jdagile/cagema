...
Class ModuleBuilderController extends Controller
{
    ...
    {
        //Show Tables
        $AllTables = DB::select("SELECT tablename FROM pg_catalog.pg_tables WHERE schemaname='public';");
        foreach ($AllTables as $tableObject) {
            //$DBkey = 'Tables_in_' . strtolower($this->dataBaseName);
           // $Table = $tableObject->$DBkey;
            $table_info_columns = DB::select(DB::raw("SELECT *  FROM information_schema.columns WHERE  table_schema = 'public' AND table_name='".$tableObject->tablename."';"));
            $FinalTables[$this->tablename] = json_encode($table_info_columns);
        }
    ...
    {
        $GetMenuList = Menus::orderBy('parent','asc')->orderBy('hierarchy','asc')->get();
    ...
    public function Modules()
    {
        ...
        foreach ($AllTables as $tableObject) {
            ...
            //$FinalTables[$Table] = json_encode($table_info_columns);
            $table_info_columns = DB::select(DB::raw("SELECT *  FROM information_schema.columns WHERE  table_schema = 'public' AND table_name='".$tableObject->tablename."';"));
            $FinalTables[$tableObject->tablename] = json_encode($table_info_columns);
        }
        $Fields = Fields::select('id', 'field_name')->where('status',1)->get()->toJson();
    ...
    {
        $Modules = Modules::all();
        return Datatables::of($Modules)->addColumn('actions', function ($Modules) {
    ...
    public function ModuleFields($ModuleID)
    {
        ...
        return Datatables::of($Fields)->addColumn('actions', function ($Fields) {
    ...
    protected function GenerateFiles()
    {
        ...
        $Model = preg_replace(array('@DummyNamespace@', '@DummyClass@', '@TableName@','@DummyOneToOneRelation@'), array('App', ucfirst($this->ModuleNameStripped), ucfirst($this->ModuleNameStripped),$OneToOneRelation), $Model);
        ...
        //Route Stub
        $Route = $this->files->get(app_path('Http/stubs/route.stub'));
        $Route = preg_replace(array('@ModuleName@'), array(ucfirst($this->ModuleNameStripped)), $Route);
        $this->files->put(app_path('Http/Routes/CrudRoutes/' . ucfirst($this->ModuleNameStripped) . '.php'), $Route);

        //Generate Crud Routes File
        $CrudRoutesFiles = $this->files->files(app_path() . '/Http/Routes/CrudRoutes');
        $CrudRoutesLines = '';
        foreach ($CrudRoutesFiles as $CrudRouteFile):
            $CrudRouteFile = pathinfo($CrudRouteFile);
            $CrudRoutesLines .= "require(app_path() . '/Http/Routes/CrudRoutes/" . $CrudRouteFile['basename'] . "');";
        endforeach;
        ...
        $this->files->put(app_path('Http/Routes/CrudRoutes.php'), $RoutesFile);
    ...
    protected function RemoveFiles()
    {
        ...
        //Remove Route Stub
        $this->files->delete(app_path('Http/Routes/CrudRoutes/' . ucfirst($this->ModuleNameStripped) . '.php'));
        //Generate Crud Routes File
    ...
    protected  function RemvoeModuleRecords(){
        //Deelete Menu Item
        Menus::where('module_id', $this->ModuleID)->delete();
        //Delete Module
